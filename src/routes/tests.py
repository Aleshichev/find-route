...
class AllTestsCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        lst = [
            ...
            Train(
                name="t6", from_city=self.city_B, to_city=self.city_A, travel_time=11
            ),
            Train(
                name="t7", from_city=self.city_A, to_city=self.city_C, travel_time=10
            ),
    ...
    def test_model_train_duplicate(self):
        ...
        train = Train(
            name="t1", from_city=self.city_A, to_city=self.city_B, travel_time=129
        )
        with self.assertRaises(ValidationError):
            train.full_clean()
    ...
    def test_model_train_train_duplicate(self):
        """Тестирование возникновения ошибки при создании дубля поезда"""
        train = Train(
            name="t1234", from_city=self.city_A, to_city=self.city_B, travel_time=9
        )
        with self.assertRaises(ValidationError):
            train.full_clean()
        try:
            ...
            self.assertEqual({"__all__": ["Змінить час"]}, e.message_dict)
            self.assertIn("Змінить час", e.messages)
    ...
    def test_cbv_detail_views(self):
        response = self.client.get(
            reverse("cities:detail", kwargs={"pk": self.city_A.id})
        )
    ...
    def test_message_error_more_time(self):
        ...
        self.assertContains(
            response, "Час у дорозі більше заданого. Збільшити час", 1, 200
        )
